...
DEFINE_BINARY_PARSER(triple_reg, {
	...
	_CT_CHECKED(directive_get_register(&instr_data->rsrc2, bin_instr,
...
DEFINE_BINARY_WRITER(triple_reg, {
	...
	_CT_CHECKED(directive_set_register(instr_data->rsrc1, bin_instr,
				FREGISTER_BIT_LEN, NO_R_HEAD_BIT));
	_CT_CHECKED(directive_set_register(instr_data->rsrc2, bin_instr,
				FREGISTER_BIT_LEN + REGISTER_BIT_LEN, NO_R_HEAD_BIT));
...
DEFINE_ASM_PARSER(triple_reg, {
	...
	_CT_CHECKED(parse_register(asm_instr->argsptrs[3], &instr_data->rsrc2));
});
